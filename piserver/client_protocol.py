...
class StreamState(common_protocol.State):
	...
	def onMessage(self, msg):
		...
		if msg['cmd'] == common_protocol.ServerCommands.DROP_TO_CONFIG:
			# that means it's wrong state and we'd better drop
			io_class = interface.get_interface_desc()
			...
			server_ackcpint = msg['ack_count']
			self.ackcount += server_ackcount
			if self.ackcount > -10:
				self.poll_and_send()

		elif msg['cmd'] == common_protocol.ServerCommands_RESUME_STREAMING:
...
class ConfigState(common_protocol.State):
	"""
	Responsible for setting up the io_class
	"""
	def onMessage(self, msg):
		msg = json.loads(msg)

		if msg['cmd'] == common_protocol.ServerCommand.CONFIG:
	...
	def instantiate_io(io_collection): # let's try to configure io
	...
	# below will be shown if something has failed
	mes = {'cmd': common_protocol.RPIClientCommands.CONFIG_OK}
